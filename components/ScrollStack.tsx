...
export const ScrollStackItem: React.FC<ScrollStackItemProps> = ({ children, itemClassName = '' }) => (
  <div
    className={`scroll-stack-card relative w-full rounded-[40px] shadow-[0_20px_50px_rgba(0,0,0,0.3)] box-border origin-top will-change-transform ${itemClassName}`.trim()}
    style={{
...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateOffsets = useCallback(() => {
    ...
    // Reset transforms temporarily to get accurate offsets
    ...
    // Re-trigger transform update
    updateCardTransforms();
  }, [getScrollData, useWindowScroll, updateCardTransforms]);
  ...
  useEffect(() => {
    ...
    const lenis = new Lenis({
      ...(useWindowScroll ? {} : { wrapper: scrollerRef.current!, content: scrollerRef.current!.querySelector('.scroll-stack-inner') as HTMLElement }),
      duration: 1.2,
  ...
  }, [itemDistance, useWindowScroll, updateOffsets, updateCardTransforms]);
  ...
  return (
    ...
    >
      <div className="scroll-stack-inner pt-[10vh] px-4 md:px-20 pb-[60vh]">
        {children}
